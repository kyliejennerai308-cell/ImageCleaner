...
# NOTE: deep_teal was removed as it was causing unwanted greenish outlines
# around yellow silhouettes during palette snapping. Edge pixels now snap
# to neon_green, bright_yellow, or sky_blue as appropriate.
PALETTE = {
...
def load_and_upscale(image_path, scale=3, use_gpu=False, gpu_backend=None):
    ...
    Args:
        ...
        scale: Upscaling factor (default 3x for better detail preservation)
    ...
    # Upscale 3x for better processing (preserves text and logo details)
...
def _remove_small_components(mask, min_area=20):
    """
    Remove tiny connected components from a binary mask using area thresholding.
    """
...
def preprocess_with_hsv(img, use_natural_green=False, skip_infill=False):
    ...
    # Scan samples (approx; scans vary by glare/dirt/light/compression):
    # - White dropshadow off pink logo: hsl(222, 60%, 83%), rgb(201, 192, 250), rgb(195, 212, 251)
    # - Text/Stars white graphics: hsl(209, 84%, 90%), hsl(209, 88%, 90%), hsl(214, 95%, 92%), hsl(212, 54%, 86%)
    ...
    # Scan samples (approx; scans vary by glare/dirt/light/compression):
    # - Blue Background: hsl(214, 74%, 70%), hsl(215, 77%, 71%)
    ...
    # Yellow hue in HSV: 56-68° HSL ≈ 28-34 OpenCV hue
    # Scan samples (approx):
    # - Yellow Inner Graphics: hsl(62, 99%, 45%), hsl(63, 100%, 45%), hsl(62, 100%, 45%)
    # Keep yellow interiors strong while leaving darker yellow-green edges for green detection.
    yellow_mask = (h >= 28) & (h <= 34) & (s > 65) & (v > 90)

    # Clean up yellow mask with area-based filtering (NOT morphological opening)
    # Uses connectedComponentsWithStats to remove tiny noise blobs while preserving
    # all fine details like hair, fingers, shoe heels, and leg gaps exactly as scanned.
    yellow_mask_filtered = _remove_small_components(yellow_mask, min_area=20)
    # Optional: light MORPH_CLOSE with tiny elliptical kernel to fill internal holes
    # without eroding outer edges
    yellow_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
    yellow_mask_processed = cv2.morphologyEx(
        yellow_mask_filtered.astype(np.uint8) * 255,
        cv2.MORPH_CLOSE,
        yellow_kernel,
        iterations=1
    )
    ...
    if skip_infill:
        ...
        yellow_mask_final = yellow_mask_processed > 0
    else:
        # Respect holes using contour tree approach
        contours, hierarchy = cv2.findContours(yellow_mask_processed, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
        yellow_filled = np.zeros_like(yellow_mask_processed)
        if hierarchy is not None:
            for i, contour in enumerate(contours):
                # If no parent, it's external boundary - fill
                if hierarchy[0][i][3] == -1:
                    cv2.drawContours(yellow_filled, [contour], -1, 255, -1)
                # If has parent, it's a hole - keep it hollow
                else:
                    cv2.drawContours(yellow_filled, [contour], -1, 0, -1)
    ...
    # ==== 4. NEON GREEN (Extracted Stroke) ====
    # Use direct HSV detection for green (preserves original green from image)
    # Green hue range: 66-81° HSL ≈ 33-41 OpenCV hue (between yellow and cyan)
    # Scan samples (approx):
    # - Green Yellow Silhouette Outline: rgb(164, 197, 26), hsl(68, 90%, 41%), hsl(73, 79%, 45%)
    # Allow yellow-green edge hues while excluding yellow interiors.
    # ADJUSTED: Expanded hue range and lowered saturation/value to catch more green edges.
    green_mask = (h >= 30) & (h <= 55) & (s > 25) & (v > 35) & ~yellow_mask_final & ~blue_mask & ~white_mask

    # Clean green mask with area-based filtering (preserves unique line shape)
    green_mask_filtered = _remove_small_components(green_mask, min_area=5)

    # Light MORPH_CLOSE to fill tiny gaps without eroding edges
    green_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
    green_mask_closed = cv2.morphologyEx(
        green_mask_filtered.astype(np.uint8) * 255,
        cv2.MORPH_CLOSE,
        green_kernel,
        iterations=1
    ) > 0

    # SYNTHETIC OUTLINE LOGIC:
    # Dig into yellow silhouettes to ensure borders are captured.
    # We'll create a synthetic outline around yellow regions that don't already have green.
    yellow_dilated = cv2.dilate(yellow_mask_final.astype(np.uint8) * 255, green_kernel, iterations=2)
    yellow_border = (yellow_dilated > 0) & ~yellow_mask_final
    
    # Combine detected green with potential border areas that are green-ish
    # This helps catch the "lost" green that might have been classified as background or yellow
    synthetic_green = yellow_border & (h >= 30) & (h <= 60) & (s > 20)

    if use_natural_green or np.any(green_mask_closed):
        print("  Using extracted and enhanced green detection")
        green_mask_final = green_mask_closed | synthetic_green
    else:
        # No synthetic outline: keep green empty if none was detected.
        green_mask_final = np.zeros_like(green_mask_closed, dtype=bool)

    img_processed[green_mask_final] = neon_green
    green_count = np.sum(green_mask_final)
    print(f"  Neon green outlines (extracted): {green_count:,} pixels → neon_green")
    
    # ==== 5. PINK/MAGENTA ELEMENTS (hot pink, outline magenta, and dark purple) ====
    # Based on provided color samples:
    # - Pink Graphic: hsl(309, 100%, 54%), hsl(310, 100%, 55%), hsl(310, 99%, 54%)
    # - Purple Outline: hsl(295, 73%, 45%), hsl(307, 99%, 43%), hsl(303, 92%, 38%)
    # - Red outline samples: hsl(0, 99%, 50%), hsl(1, 99%, 50%), hsl(0, 100%, 50%), hsl(5, 100%, 41%)
    ...
    contours, hierarchy = cv2.findContours(pink_uint8, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    pink_final_mask = np.zeros_like(pink_uint8)
    if hierarchy is not None:
        for i, contour in enumerate(contours):
            # If the contour has no parent, it's an external boundary (parent logo shape)
            if hierarchy[0][i][3] == -1:
                cv2.drawContours(pink_final_mask, [contour], -1, 1, -1)
            # If it has a parent, it's a hole (text inside logo) - clear it
            else:
                cv2.drawContours(pink_final_mask, [contour], -1, 0, -1)
    else:
        pink_final_mask = pink_uint8 > 0

    img_processed[pink_final_mask > 0] = hot_pink
    img_processed[outline_magenta_mask] = outline_magenta
    img_processed[dark_purple_mask] = dark_purple
    
    pink_count = np.sum(pink_final_mask > 0)
...
def bilateral_smooth_edges(img, d=9, sigma_color=50, sigma_space=50, use_gpu=False, gpu_backend=None, preserve_mask=None):
    ...
    lighter touch for text/logos (Pink/White areas).
    """
    print("Applying mask-based edge-preserving smoothing...")
    
    # Create mask for White and Pink areas to protect details
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    h, s, v = cv2.split(hsv)
    # White: low saturation, high value. Pink: high saturation, specific hue.
    detail_mask = ((s < 60) & (v > 180)) | (((h >= 140) & (h <= 175)) & (s > 70))
    
    # Aggressive smoothing for non-detail areas
    img_aggressive = cv2.bilateralFilter(img, d=15, sigmaColor=75, sigmaSpace=75)
    
    # Lighter touch for detail areas (original or light blur)
    img_light = cv2.bilateralFilter(img, d=5, sigmaColor=30, sigmaSpace=30)
    
    # Blend based on detail_mask
    mask_3ch = cv2.cvtColor(detail_mask.astype(np.uint8) * 255, cv2.COLOR_GRAY2BGR).astype(np.float32) / 255.0
    smoothed = (mask_3ch * img_light.astype(np.float32) + (1 - mask_3ch) * img_aggressive.astype(np.float32)).astype(np.uint8)
...
def detect_text_regions(img):
    ...
    dilated = cv2.dilate(edges, text_kernel, iterations=1)
    ...
    edge_buffer = cv2.dilate(high_contrast, morph_kernel, iterations=1)
...
def morphological_cleanup(
    ...
    UPDATED: Protect white and pink text regions by skipping MORPH_OPEN entirely for them.
    Only MORPH_CLOSE is applied to white/pink areas (to fill holes without deleting thin lines).
    """
    ...
    # SKIP MORPH_OPEN for white/pink pixels to protect thin text and logo details.
    opened = cv2.morphologyEx(img, cv2.MORPH_OPEN, kernel, iterations=1)
    
    # Restore white and pink pixels that were eroded by MORPH_OPEN
    opened[protected_pixel_mask] = img[protected_pixel_mask]
    ...
    if area_min and area_min > 0:
        sky_blue = np.array(PALETTE['sky_blue'], dtype=np.uint8)
        yellow_min_area = area_min
        green_min_area = max(5, area_min // 4)
        color_thresholds = {
            'bright_yellow': yellow_min_area,
            'neon_green': green_min_area,
        }
        for color_name, min_area in color_thresholds.items():
            color_bgr = np.array(PALETTE[color_name], dtype=np.uint8)
            color_mask = np.all(cleaned == color_bgr, axis=2)
            color_mask_clean = _remove_small_components(color_mask, min_area=min_area)
            removed = color_mask & ~color_mask_clean
            cleaned[removed] = sky_blue
...
def edge_preserving_smooth(img, sigma_color=75, sigma_space=75, preserve_mask=None):
    ...
    UPDATED: Also preserve white text pixels from smoothing.
    """
    print("Applying edge-preserving smoothing for clean outlines...")
    
    # Create white text protection mask - preserve white pixels from smoothing using palette constant
    pure_white = np.array(PALETTE['pure_white'], dtype=np.uint8)
    white_pixel_mask = np.all(img == pure_white, axis=2)
    ...
    # Restore white text pixels
    smoothed[white_pixel_mask] = img[white_pixel_mask]
...
def apply_anti_aliasing(img, preserve_mask=None):
    ...
    UPDATED: Also preserve white text pixels from anti-aliasing.
    """
    ...
    # Restore white text pixels
    result[white_pixel_mask] = img[white_pixel_mask]
...
def vectorize_edges(img, straightness_threshold=0.001, min_contour_area=500):
    ...
    Uses low threshold (0.001) to preserve organic curves (hands, feet) 
    while still sharpening blocky edges.
    Treats outline colors as STROKES, not filled regions.
    """
    ...
    # SKIP neon_green - preserve natural green outline from original image
    outline_color_names = {
        'outline_magenta',
        'dark_purple',
        'vibrant_red'
    ...
    # Process each palette color (except background, black, neon_green, pure_white,
    # and bright_yellow to avoid squaring organic silhouette details like feet)
    for color_name, color_bgr in PALETTE.items():
        # Skip background, black, neon_green (preserve natural green outline),
        # pure_white (preserve text detail - approxPolyDP loses thin strokes),
        # and bright_yellow (preserve scanned silhouette edges)
        if color_name in ['sky_blue', 'black', 'neon_green', 'pure_white', 'bright_yellow']:
            continue
    ...
    print(f"  Processed {total_contours_processed} contours (Clean Vector)")
    return img_result
...
def smooth_jagged_edges(img):
    ...
    Uses small kernels to preserve detail while smoothing edges.
    """
    ...
    for color_name, color_bgr in PALETTE.items():
        ...
        # Apply morphological smoothing with small kernel to preserve details
        ...
        if color_name in outline_colors_to_process:
            ...
            # FILLS: opening + closing for solid regions
        ...
        # Apply light Gaussian blur then threshold to smooth edges
...
def solidify_color_regions(img, kernel_size=3, preserve_mask=None):
    """
    Apply median filter within each color region to remove gradients and texture.
    Smaller kernel (3 vs 5) to preserve detail better.
    UPDATED: Apply median blur per-mask only (not as a global pre-processing step)
    to keep star edges sharp.
    """
    print(f"Solidifying color regions (median kernel={kernel_size})...")
    
    img_solid = img.copy()
    
    # Pre-compute blurred version for mask-level application only
    # This is NOT applied globally — it's only used within individual color masks
    blurred = cv2.medianBlur(img, kernel_size)
    
    # For each palette color, find regions and apply solidification
    for color_name, color_bgr in PALETTE.items():
        # Skip white to prevent bleeding into blue and preserve text detail,
        # and hot_pink to preserve small logo details with white shading
        if color_name in ['pure_white', 'hot_pink']:
            continue
        ...
        if num_pixels > 500:
            # Apply blurred version only within this mask's region
...
def apply_green_outline_from_yellow(img, ring_size=7, blue_neighbor_size=5, seed_mask=None):
    """
    ...
    # Use subtract (dilated - original) so green is drawn BEHIND yellow,
    # not eating into yellow pixels. This prevents "choking" the silhouette.
    ...
    green_fringe = cv2.subtract(yellow_dilated, yellow_mask.astype(np.uint8) * 255)
    yellow_edge = green_fringe > 0
...
def restore_image(image_path, output_dir, use_gpu=False, gpu_backend=None, skip_outline_normalization=False, skip_despec=False, use_natural_green=False, skip_infill=False):
    ...
    green_outline_mask_small = _remove_small_components(green_outline_mask_small, min_area=5)
    green_close_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
    green_outline_mask_small = cv2.morphologyEx(
        green_outline_mask_small.astype(np.uint8) * 255,
        cv2.MORPH_CLOSE,
        green_close_kernel,
        iterations=1
    ) > 0
    ...
    yellow_final_mask = np.all(img_final == bright_yellow, axis=2)
    pink_final_mask = np.all(img_final == hot_pink, axis=2)
    magenta_final_mask = np.all(img_final == outline_magenta, axis=2)
    purple_final_mask = np.all(img_final == dark_purple, axis=2)
    protected_mask = yellow_final_mask | pink_final_mask | magenta_final_mask | purple_final_mask

    green_apply_mask = green_outline_mask_small & ~protected_mask
    img_final[green_apply_mask] = neon_green
    img_final[yellow_final_mask] = bright_yellow
    img_final[pink_final_mask] = hot_pink
    img_final[magenta_final_mask] = outline_magenta
    img_final[purple_final_mask] = dark_purple
    
    # Phase 10: Priority stamp white layer last
